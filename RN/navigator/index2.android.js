...
class List extends Component {
  ...
  render() {
    if (this.state.user) {
      return(
        <View>
          <Text state={styles.list_item}>{JSON.stringify(this.state.user)}</Text>
        </View>
      );
    } else {
      return (
...
class Detail extends Component {
  ...
  render() {
    return (
      <ScrollView>
        <Text style={styles.list_item} onPress={this._pressButton.bind(this)}>这是从上一个页面传来的：{this.state.id}</Text>
      </ScrollView>
    );
  }
}
